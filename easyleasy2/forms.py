...
class Support_form(forms.Form):
    ...
    topic = forms.CharField(widget=forms.TextInput(
        attrs={'class': 'topic', 'placeholder': 'Тема запроса/продукт', 'id':'topic'}))
    name = forms.CharField(widget=forms.TextInput(attrs={'class': 'name', 'placeholder': 'Ваше имя', 'id':'name'}))
    phone_num = forms.CharField(widget=forms.TextInput(attrs={'class': 'phone_num', 'placeholder': 'Номер телефонаl', 'id':'phone_number'}))
