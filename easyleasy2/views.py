...
def support_success_page(request):
    for parent, dirnames, filenames in os.walk('../easyleasy'):
        for fn in filenames:
            if fn.lower().endswith('.pdf'):
                os.remove(os.path.join(parent, fn))
    ...
    for parent, dirnames, filenames in os.walk('../easyleasy'):
        for fn in filenames:
            if fn.lower().endswith('.pdf'):
                os.remove(os.path.join(parent, fn))
    if request.method == 'GET':
        return render(request, 'admin_support_requests_page.html')
    # else:
...
def deeds_page(request):
    for parent, dirnames, filenames in os.walk('../easyleasy'):
        for fn in filenames:
            if fn.lower().endswith('.pdf'):
                os.remove(os.path.join(parent, fn))
    return render(request, 'admin_deeds_page.html')
